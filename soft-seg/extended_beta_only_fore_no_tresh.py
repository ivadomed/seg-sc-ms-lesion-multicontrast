...
def main_extended_beta_calibration(input_msd, path_model, output_folder, smaller_changes=False, calib_model_path=None):
    ...
    for img, label in tqdm(zip(eval_images, eval_labels), desc="Running lesion segmentation on evaluation set"):
        ...
        for down_factor in downsampling_factors:
            ...
            calibrated_probs_thresholded = np.where(probs > 0.5, calibrated_probs, 0)
    ...
    text_to_write += f"Dice before beta calibration: {dice_before_calib.mean()}\n"
    text_to_write += f"Dice after beta calibration: {dice_after_calib.mean()}\n"
    text_to_write += f"Mean lesion volume for soft predictions before beta calibration: {lesion_volume_mean_soft_before.mean()}\n"
    text_to_write += f"Mean lesion volume for soft predictions after beta calibration: {lesion_volume_mean_soft_after.mean()}\n"
    text_to_write += f"Mean lesion volume for binary predictions before beta calibration: {lesion_volume_mean_binary_before.mean()}\n"
    text_to_write += f"Mean lesion volume for binary predictions after beta calibration: {lesion_volume_mean_binary_after.mean()}\n"
