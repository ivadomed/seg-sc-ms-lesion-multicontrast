...
def main_isotonice_reg(input_msd, path_model, output_folder, smaller_changes=False, path_iso=None):
    ...
    lesion_volume_soft_before = results_df.groupby("subject")["soft_lesion_volume_before_iso"].mean()
    lesion_volume_binary_before = results_df.groupby("subject")["bin_lesion_volume_before_iso"].mean()
    lesion_volume_soft_after = results_df.groupby("subject")["soft_lesion_volume_after_iso"].mean()
    lesion_volume_binary_after = results_df.groupby("subject")["bin_lesion_volume_after_iso"].mean()
    ...
    ## Dice before and after isotonic regression
    dice_before_iso = results_df.groupby("subject")["dice_before_iso"].mean()
    dice_after_iso = results_df.groupby("subject")["dice_after_iso"].mean()
    ...
    text_to_write += f"Mean soft lesion volume before isotonic regression: {lesion_volume_soft_before.mean()}\n"
    text_to_write += f"Mean soft lesion volume after isotonic regression: {lesion_volume_soft_after.mean()}\n"
    text_to_write += f"Mean binary lesion volume before isotonic regression: {lesion_volume_binary_before.mean()}\n"
    text_to_write += f"Mean binary lesion volume after isotonic regression: {lesion_volume_binary_after.mean()}\n"
    text_to_write += f"Dice before isotonic regression: {dice_before_iso.mean()}\n"
